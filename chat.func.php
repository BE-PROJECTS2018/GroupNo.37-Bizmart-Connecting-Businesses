...
$query="SELECT Sender , Message ,Time FROM chat WHERE ((Sender='$sender' AND Receiver='$receiver') OR (Sender='$receiver' AND Receiver='$sender') ) AND Pid=".$_SESSION["c_p_id"]."  ORDER BY Msg_id";
...
$query="SELECT DISTINCT Userid  FROM biddet WHERE Pid=".$pid." AND Ownerid=".$sender;
...
$query="SELECT DISTINCT Ownerid FROM biddet WHERE Pid=".$pid." AND Userid=".$sender;
